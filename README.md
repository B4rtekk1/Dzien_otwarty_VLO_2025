...
Sieci neuronowe są oparte na losowośći. Losowość sprawia, że każda sieć neuronowa jest wyjątkowa. Sieć zaczyna naukę w innym miejscu, przez co otrzymujemy różne rozwiazania, wzory, dokładność. 
...
```python
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # Sprawdza czy komputer posiada kartę graficzną Nvidii
